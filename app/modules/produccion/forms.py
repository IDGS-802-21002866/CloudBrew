...
class ProduccionForm(FlaskForm):
    id_receta = SelectField("Receta", coerce=int, validators=[DataRequired()])
...
class ProduccionProcesoForm(FlaskForm):
    id_proceso = SelectField(
        "Proceso Productivo", coerce=int, validators=[DataRequired()]
    )

    fecha_inicio = DateField(
        "Fecha Inicio", format="%Y-%m-%d", validators=[DataRequired()]
    )
